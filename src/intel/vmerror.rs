...
impl core::fmt::Display for VmxBasicExitReason {
	...
	fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
		let description = match *self {
			...
			VmxBasicExitReason::IoSystemManagementInterrupt => {
			...
			VmxBasicExitReason::MovDr => {
				"Guest software attempted a MOV to or from a debug register"
			}
			VmxBasicExitReason::IoInstruction => {
				"Guest software attempted to execute an I/O instruction"
			}
			...
			VmxBasicExitReason::Pause => {
				"Either guest software attempted to execute PAUSE or the PAUSE-loop exiting VM-execution control was 1"
			}
			...
			VmxBasicExitReason::WbinvdOrWbnoinvd => {
				"Guest software attempted to execute WBINVD or WBNOINVD"
			}
			...
			VmxBasicExitReason::Vmfunc => {
				"Guest software invoked a VM function with the VMFUNC instruction"
			}
			...
			VmxBasicExitReason::EnqcmdsPasidTranslationFailure => {
